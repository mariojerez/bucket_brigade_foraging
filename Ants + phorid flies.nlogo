...
to setup
  ...
  create-foragers starting-foraging-population [
    set size 2         ;; easier to see
    set color red     ;; red = not carrying food
    setxy 30 -30
    set vulnerable? false
    set hitchnum 0 ]
  create-minims hitchhiking-population [
    set partner nobody
    set size 1
    set color white
    setxy 30 -30 ]
...
to setup-nest  ;; patch procedure
  ...
  set nest? (distancexy 30 -30) < 5
  ;; spread a nest-scent over the whole world -- stronger near the nest
  set nest-scent 200 - distancexy 30 -30
end
...
to check-death
  if vulnerable?
  [if random 100 > 100 - parasite-intensity
    [ die ]]
end

to return-to-nest  ;; turtle procedure
  ;ifelse linked-to-minimant?
  ifelse nest?
...
to look-for-transporter
  ifelse any? (foragers-here with [ vulnerable? ]) [
    set partner one-of (foragers-here with [ vulnerable? ])
    ask partner [
      set vulnerable? false
      set color green
      set hitchnum hitchnum + 1
      ask myself [move-to partner]]][
...
When a foraging ant (a big ant) finds a piece of food, it carries the food back to the nest, dropping a chemical as it moves. When other ants "sniff" the chemical, they follow the chemical toward the food. As more ants carry food to the nest, they reinforce the chemical trail.

Minims (the small ants) attach themselves to forager ants that are carrying food back to the nest. When a foraging ant carries food back to the nest, it has a certain probability (set by the parasite-level slider) of dying for every timestep unless a minim protects it by attaching itself to it; in that case the ant transporting food to the nest turns green has a 100% chance of survival and.
...
The PARASITE-LEVEL slider controls the probability that a foraging ant will die in each timestep when carrying food.
...
The population of forager ants is shown in a plot. When the parasite level is higher, you will notice that a higher minim population is required to sustain the foraging population.
